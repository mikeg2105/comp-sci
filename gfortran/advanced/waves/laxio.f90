...
!write data file
subroutine write_file(filename,x,y,z)
    character(len=20), intent(in) :: filename
    double precision, intent(in) :: x(:),y(:),z(:)  ! arrays will automatically take size of input array
    integer :: i,iu
    character(len=30) :: fmtstring ="(es12.6,1x,es12.6,1x,es12.6)"
    ...
    do i=1,min(size(x),size(y),size(z))
        write(iu,fmtstring) x(i),y(i),z(i)
    enddo
    close(unit=iu)
